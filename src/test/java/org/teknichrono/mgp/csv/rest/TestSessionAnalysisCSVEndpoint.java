...
class TestSessionAnalysisCSVEndpoint {
  ...
  void getPracticeAnalysis() throws IOException, CsvException {
    ...
    try (CSVReader reader = new CSVReaderBuilder(new StringReader(csv)).withSkipLines(1).build()) {
      ...
      assertThat(Integer.parseInt(allData.get(0)[10])).isEqualTo(0);
      assertThat(Integer.parseInt(allData.get(0)[11])).isEqualTo(0);
      assertThat(Boolean.valueOf(allData.get(0)[12])).isEqualTo(false);
      assertThat(Boolean.valueOf(allData.get(0)[13])).isEqualTo(false);
    }
